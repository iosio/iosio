...
export const rollupConfig =
    ({
         buildType = 'dev', cwd, name,  entry, alias, compress, html,
         output, browsers, cssBrowsers, commonjsConfig, copyConfig, sourcemap, target, writeMeta,
         define, format, jsx, jsxFragment, entries = [], pkg, external = 'none', globals, strict, raw, legacy,
     }) => {



        const DEV = buildType === 'dev';
        const BUILD_APP = buildType === 'build_app';
        const BUILD_LIB = buildType === 'build_lib';
        ...
        if (multipleEntries) {
            let name_ = entry.match(/([\\/])index(\.(umd|cjs|es|m))?\.m?js$/)
                ? mainNoExtension
                : entry;
            mainNoExtension = resolve(dirname(mainNoExtension), basename(name_));
        }
