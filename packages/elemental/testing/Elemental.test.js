...
let calledBeforeInitialUpdate = 0;

class ElementalElement extends Elemental {
    static propTypes = propTypes;
    static shadow = true;
    static template = '<h1>hello <span data-name> </span></h1>';
    static styles = ':host{--color:red;}h1{color:var(--color)}';
    static proxyRefs = {selector: r => `[data-${r}]`};
...
export const ElementalTest = () => {
    webComponentTester({
        tag: 'elemental-element',
        Class: ElementalElement,
        tests: [
...
when values were changed ${numTimesValueSet} time${(numTimesValueSet > 1 || numTimesValueSet === 0) ? 's' : ''};`
                ...
                it('updates a ref using propLogic', () => {
                    expect(c.refs.name.textContent).toBe('asdf');
                });
                ...
                await until(500);
                it('should have called beforeInitialUpdate once', () => {
                    expect(calledBeforeInitialUpdate).toBe(1);
                });
